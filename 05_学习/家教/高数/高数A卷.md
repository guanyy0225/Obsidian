...
*   **解答:**
    ...
    *   **当 $0<a<1$ 时:** $\log_a(x^2+y^2) \ge 0 \implies 0 < x^2+y^2 \le a^0 = 1$。
...
想象一条由参数方程定义的平滑曲线。我们可以把这条曲线看作是由无数个**极小、极小的直线段**拼接而成的。我们只要求出每一小段直线的长度 `ds`，然后用积分把它们全部“加”起来，就是整条曲线的弧长 `L`。

**如何计算一小段的长度 `ds`？**

1.  **放大观察:** 当我们把曲线放大到极致，一小段弯曲的弧线 `ds` 就近似于一条直线段。
2.  **构建微小直角三角形:** 在极小的一段时间 `dt` 内，点在曲线上移动了 `ds` 的距离。这个移动可以分解为：
    *   在 **x 方向** 移动了一个微小的距离 `dx`。
    *   在 **y 方向** 移动了一个微小的距离 `dy`。
    
    `ds`, `dx` 和 `dy` 构成了一个微小的直角三角形，其中 `ds` 是斜边。
...
4.  **引入参数 `t`:**
    ...
    *   $dy = y'(t) dt = \frac{dy}{dt} dt$
    ...
    将这两个式子代入上面的 `ds` 表达式：
    $$
    ds = \sqrt{\left(\frac{dx}{dt} dt\right)^2 + \left(\frac{dy}{dt} dt\right)^2} = \sqrt{\left[\left(\frac{dx}{dt}\right)^2 + \left(\frac{dy}{dt}\right)^2\right] (dt)^2}
    $$
    将 $dt$ 从根号中提出来，就得到了**弧长元素 (Arc Length Element)** 的公式：
    $$
...
如果曲线 L 由参数方程 $\begin{cases} x = x(t) \\ y = y(t) \end{cases} \quad (\alpha \le t \le \beta)$ 给出，其弧长为：
$$
...
这是**判断级数发散**的第一道防线，也是最简单的必要条件。

*   **内容**：对于级数 $\sum_{n=1}^{\infty} a_n$，如果通项 $a_n$ 的极限**不等于 0** 或者**极限不存在**，即 $\lim_{n \to \infty} a_n \neq 0$，那么该级数**必定发散**。
*   **注意**：反过来不成立！如果 $\lim_{n \to \infty} a_n = 0$，级数**可能收敛也可能发散**。例如，$\sum \frac{1}{n}$ 的通项趋于0，但级数发散。
*   **何时使用**：拿到任何一个级数，**首先心算一下它的通项极限**。如果极限不为0，直接判定发散，问题解决。

---

### **第二步：针对特定类型的级数**

如果通项极限为0，我们就需要根据级数的具体形式选择更精细的判别法。

#### 2. 几何级数 / p-级数

这是两种最基本的、可以直接得出结论的级数，很多其他判别法（比如比较判别法）都是将复杂级数与这两种级数进行比较。

*   **几何级数**: $\sum_{n=1}^{\infty} ar^{n-1}$
    *   **判别**：
        *   当公比 $|r| < 1$ 时，级数**收敛**，和为 $S = \frac{a}{1-r}$。
        *   当公比 $|r| \ge 1$ 时，级数**发散**。

*   **p-级数**: $\sum_{n=1}^{\infty} \frac{1}{n^p}$
    *   **判别**：
        *   当 $p > 1$ 时，级数**收敛**。
        *   当 $p \le 1$ 时，级数**发散**。（其中 $p=1$ 的情况是著名的**调和级数** $\sum \frac{1}{n}$，它发散）

---

### **第三步：针对正项级数**

如果级数 $\sum a_n$ 的所有项 $a_n \ge 0$，我们可以使用以下几种强大的判别法。

#### 3. 比较判别法

*   **思想**：将待判定的级数与一个已知的、更简单的级数（通常是 p-级数或几何级数）进行比较。
*   **内容**：设 $\sum a_n$ 和 $\sum b_n$ 都是正项级数。
    *   **“大的收敛，小的必收敛”**: 如果 $\sum b_n$ 收敛，且 $a_n \le b_n$，则 $\sum a_n$ 也**收敛**。
    *   **“小的发散，大的必发散”**: 如果 $\sum b_n$ 发散，且 $a_n \ge b_n$，则 $\sum a_n$ 也**发散**。
*   **何时使用**：当级数的通项可以很方便地进行放大或缩小时。
*   **示例**: 判断 $\sum \frac{1}{n^2+1}$ 的敛散性。因为 $\frac{1}{n^2+1} < \frac{1}{n^2}$，而 $\sum \frac{1}{n^2}$ 是收敛的p-级数($p=2>1$)，所以原级数收敛。

#### 4. 极限比较判别法

这是比较判别法的变体，通常更强大、更容易使用。

*   **思想**：通过取极限来判断两个级数的“增长速度”是否相当，从而判定它们具有相同的敛散性。
*   **内容**：设 $\sum a_n$ 和 $\sum b_n$ 都是正项级数，计算极限 $L = \lim_{n \to \infty} \frac{a_n}{b_n}$。
    *   如果 $0 < L < \infty$（即 L 是一个有限的正数），则两个级数**同敛散**。
    *   如果 $L=0$ 且 $\sum b_n$ 收敛，则 $\sum a_n$ 收敛。
    *   如果 $L=\infty$ 且 $\sum b_n$ 发散，则 $\sum a_n$ 发散。
*   **何时使用**：当通项是多项式或根式的比值时，通过只保留最高次幂项来构造简单的比较级数 $b_n$。
*   **示例**: 判断 $\sum \frac{n}{2n^2-1}$ 的敛散性。我们观察到当 n 很大时，$a_n = \frac{n}{2n^2-1} \approx \frac{n}{2n^2} = \frac{1}{2n}$。所以我们选择比较级数 $\sum b_n = \sum \frac{1}{n}$ (发散)。
    $\lim_{n \to \infty} \frac{a_n}{b_n} = \lim_{n \to \infty} \frac{n/(2n^2-1)}{1/n} = \lim_{n \to \infty} \frac{n^2}{2n^2-1} = \frac{1}{2}$。因为 $0 < 1/2 < \infty$，所以原级数与 $\sum \frac{1}{n}$ 同敛散，即**发散**。

#### 5. 比值判别法

*   **思想**：通过考察相邻两项的比值来判断级数通项的增长趋势，类似于几何级数。
*   **内容**：计算极限 $L = \lim_{n \to \infty} \left| \frac{a_{n+1}}{a_n} \right|$。
    *   如果 $L < 1$，级数**绝对收敛**。
    *   如果 $L > 1$ 或 $L=\infty$，级数**发散**。
    *   如果 $L = 1$，此方法**失效**，需要换用其他方法。
*   **何时使用**：当通项中含有**阶乘 $n!$** 或**指数项 $k^n$** 时，比值法通常非常有效。

#### 6. 根值判别法

*   **思想**：通过考察通项的 n 次方根来判断其增长趋势。
*   **内容**：计算极限 $L = \lim_{n \to \infty} \sqrt[n]{|a_n|}$。
    *   如果 $L < 1$，级数**绝对收敛**。
    *   如果 $L > 1$ 或 $L=\infty$，级数**发散**。
    *   如果 $L = 1$，此方法**失效**。
*   **何时使用**：当通项整体是 **$n$ 次幂** 的形式，如 $(\dots)^n$ 时，根值法最简单。

#### 7. 积分判别法

*   **内容**：如果 $a_n = f(n)$，其中 $f(x)$ 是一个**连续、正、递减**的函数，那么级数 $\sum a_n$ 和反常积分 $\int_1^\infty f(x)dx$ **同敛散**。
*   **何时使用**：当通项很容易被积出来时。p-级数的敛散性就是通过这个方法证明的。

---

### ==**第四步：针对交错级数**==

如果级数 $\sum a_n$ 的项正负交替出现，即 $\sum (-1)^{n-1} b_n$ ($b_n > 0$)。

#### 8. 莱布尼茨判别法 

*   **内容**：对于交错级数 $\sum (-1)^{n-1} b_n$，如果满足以下**两个条件**：
    1.  $\lim_{n \to \infty} b_n = 0$
    2.  $b_{n+1} \le b_n$ (即 $b_n$ 序列单调递减)
    那么级数**收敛**。

*   **应用到你的题目**：$\sum \frac{(-1)^{n-1}}{n^p}$。这里 $b_n = \frac{1}{n^p}$。
    *   当 $p>0$ 时，$\lim \frac{1}{n^p} = 0$ 且 $\frac{1}{(n+1)^p} \le \frac{1}{n^p}$，两个条件都满足，所以级数**收敛**。
    *   当 $p \le 0$ 时，$\lim \frac{1}{n^p} \neq 0$，不满足第一个条件，所以不能用莱布尼茨判别法。但我们可以回到第一步的通项判别法，因为通项 $(-1)^{n-1}/n^p$ 的极限不存在（在正负之间振荡且绝对值不趋于0），所以级数**发散**。
    *   因此，级数发散的条件是 $p \le 0$。

### **绝对收敛与条件收敛**

对于一般项级数（有正有负），我们还需要判断其收敛类型。

*   **绝对收敛 (Absolutely Convergent)**：如果级数各项取绝对值后得到的正项级数 $\sum |a_n|$ 收敛，则称原级数 $\sum a_n$ 绝对收敛。**绝对收敛必定收敛**。
*   **条件收敛 (Conditionally Convergent)**：如果级数 $\sum a_n$ 本身收敛，但其绝对值级数 $\sum |a_n|$ 发散，则称原级数 $\sum a_n$ 条件收敛。
    *   **典型例子**：交错调和级数 $\sum \frac{(-1)^{n-1}}{n}$。它本身收敛（根据莱布尼茨判别法），但它的绝对值级数 $\sum \frac{1}{n}$ 发散。

### **判断流程图**

1.  **看通项** $\lim_{n \to \infty} a_n \neq 0$? $\implies$ **发散**。
2.  是**几何级数**或**p-级数**? $\implies$ 直接用结论。
3.  是**正项级数**?
    *   形式简单 $\implies$ **比较判别法** / **极限比较判别法**。
    *   含 $n!$ 或 $k^n$ $\implies$ **比值判别法**。
    *   含 $(\dots)^n$ $\implies$ **根值判别法**。
    *   容易积分 $\implies$ **积分判别法**。
4.  是**交错级数**? $\implies$ **莱布尼茨判别法**判断是否收敛。
5.  如果收敛，再看**绝对值级数** $\sum |a_n|$ 是否收敛（用正项级数的方法判断）。
    *   $\sum |a_n|$ 收敛 $\implies$ **绝对收敛**。
    *   $\sum |a_n|$ 发散 $\implies$ **条件收敛**。
...
*   **(A) $f(x,y)$ 在 $(x_0,y_0)$ 处连续**
    *   **分析：** 连续是可微的**必要条件**，但不是充分条件。可微一定连续，但连续不一定可微（例如圆锥顶点 $z=\sqrt{x^2+y^2}$）。所以 (A) 错误。

*   **(B) $f'_x(x,y), f'_y(x,y)$ 在 $(x_0,y_0)$ 的某邻域内存在**
    *   **分析：** 仅仅存在偏导数甚至不能保证函数连续，更不能保证可微。所以 (B) 错误。

*   **(C) $\Delta z - f'_x(x_0,y_0)\Delta x - f'_y(x_0,y_0)\Delta y$ 当 $\sqrt{(\Delta x)^2+(\Delta y)^2} \to 0$ 时，是无穷小**
    *   **分析：** 这个选项用数学语言写出来是 $\lim_{\rho \to 0} (\Delta z - f'_x\Delta x - f'_y\Delta y) = 0$ (其中 $\rho = \sqrt{(\Delta x)^2 + (\Delta y)^2}$)。
    *   如果函数可微，那么这个条件是成立的。但反过来，这个条件能推出可微吗？**不能**。这个条件仅仅是**函数连续**的一个等价叙述（在偏导数存在的前提下）。它比可微的定义要弱。所以 (C) 错误。
...
*   **(D) $\lim_{\substack{\Delta x \to 0 \\ \Delta y \to 0}} \frac{\Delta z - f'_x(x_0,y_0)\Delta x - f'_y(x_0,y_0)\Delta y}{\sqrt{(\Delta x)^2 + (\Delta y)^2}} = 0$**
    *   **分析：** 这个表达式**正是函数 $f(x,y)$ 在点 $(x_0, y_0)$ 处可微的定义**。
    *   定义本身是一个**充要条件**。
...
*   **思路:** 参数方程的弧长公式
* 若曲线由参数方程 $\begin{cases} x = x(t) \\ y = y(t) \end{cases} \quad (\alpha \le t \le \beta)$ 给出，其弧长 $L$ 为：
$$
...
*   **分析:**
    $\lim_{n \to \infty} nu_n = 0$ 意味着 $u_n$ 比 $1/n$更快地趋向于0。
...
**1. 设函数 $f(x)$ 在 $(-\infty, +\infty)$ 内单调有界，若数列 $\{x_n\}$ [ A ]，则 $\{f(x_n)\}$ 收敛。**

*   **思路:** 这是数列收敛与函数极限关系中的**单调有界定理**的应用。
*   **分析:**
    *   单调有界数列必有极限（即必收敛）。
...
**2. 设 $f(x)$ 在 $(-\infty, +\infty)$ 内有定义，且 $\int_a^b f(x)dx$ 存在，则 $f(x)$ 在区间 $[a,b]$ 上 [ D ]**

*   **思路:** 考察定积分存在的条件。
*   **分析:**
    *   (A) 连续
    *   (B) 可导
    *   (C) $\frac{1}{b-a}\int_a^b f(x)dx \ge f(a)$: 没有任何依据。
    *   (D) $\left|\int_a^b f(x)dx\right| \le \int_a^b |f(x)|dx$: 这是定积分的一个基本性质，称为积分不等式性质。只要 $f(x)$ 可积，$|f(x)|$ 也可积，且该不等式恒成立。


连续、可导、可积、可偏导以及可微之间的关系

我们将从**一元函数**和**多元函数**两个层面来讨论，因为它们之间的关系有很大的不同。

---

### 一元函数：$y = f(x)$

对于定义在区间上的一元函数，它们的关系构成了一个相对简单的“鄙视链”。

#### 关系链条

**可导 $\implies$ 连续 $\implies$ 可积**
...
1.  **可导 $\implies$ 连续**
    *   **含义**：如果一个函数在某点是可导的，那么它在该点必定是连续的。
    *   **直观理解**：函数图像在某点有确定的、不垂直的切线，意味着图像在该点必然是“连接”在一起的，不能有断点。
    *   **反例（连续 $\not\implies$ 可导）**：$f(x) = |x|$ 在 $x=0$ 处。它是连续的（图像没有断开），但在该点形成一个尖点，无法做出唯一的切线，因此不可导。

2.  **连续 $\implies$ 可积**
    *   **含义**：如果一个函数在闭区间 $[a,b]$ 上是连续的，那么它在该区间上必定是可积的（黎曼可积）。
    *   **直观理解**：连续函数的图像是“完好无损”的曲线，我们可以用无数个小矩形去逼近它下面的面积，并且这个逼近过程是收敛的。
    *   **反例（可积 $\not\implies$ 连续）**：含有有限个**跳跃间断点**的分段函数。例如：
        $f(x) = \begin{cases} 1, & 0 \le x \le 1 \\ 2, & 1 < x \le 2 \end{cases}$
        这个函数在 $x=1$ 处不连续，但它在 $[0,2]$ 上是可积的。我们可以明确算出它下面的面积是 $1 \times 1 + 2 \times 1 = 3$。
...
多元函数的情况要复杂得多，因为“方向”变多了。**偏导数**的出现打破了一元函数中简洁的线性关系。

**关键结论：**

1.  **可微 $\implies$ 连续**
2.  **可微 $\implies$ 偏导数存在**
3.  **偏导数连续 $\implies$ 可微** (这是一个非常重要的**充分条件**)
4.  **连续 $\not\implies$ 偏导数存在，反之亦然** (连续和偏导数存在之间**没有必然联系**)
5.  **（连续 + 偏导数存在） $\not\implies$ 可微** (即使两者都满足，也不一定可微)

#### 详细解释与示例

1.  **可微**
    *   **含义**：函数在某点附近可以用一个**切平面**很好地近似。这是最强的光滑性要求。
    *   **可微 $\implies$ 连续**: 如果能用一个光滑的平面去贴近函数曲面，那么这个曲面本身不可能是断开的。
    *   **可微 $\implies$ 偏导数存在**: 既然存在一个切平面，那么沿着坐标轴方向的切线（偏导数）也必然存在。

2.  **偏导数连续 
    *   **含义**: 函数的偏导数 $f'_x$ 和 $f'_y$ 不仅存在，而且它们本身也是连续函数。
    *   **偏导数连续 $\implies$ 可微**: 这是判断一个多元函数是否可微的**最常用、最实用的充分条件**。它保证了函数在所有方向上的变化都是平滑过渡的，从而确保了切平面的存在。

3.  **偏导数存在 
    *   **含义**：函数仅仅在**沿着坐标轴**的几个特定方向上存在变化率。
    *   **偏导数存在 $\not\implies$ 连续**: 这是多元函数最反直觉的特性之一。
        *   **经典反例**：$f(x, y) = \begin{cases} \frac{xy}{x^2 + y^2} & (x,y) \neq (0,0) \\ 0 & (x,y) = (0,0) \end{cases}$
        *   这个函数在原点的两个偏导数都存在且为 0，但函数在原点**不连续**（从不同路径趋近原点，极限值不同）。
        *   **原因**：偏导数只看到了“东”和“北”方向的情况，却对“东北”方向的“悬崖”一无所知。

4.  **连续 
    *   **含义**：函数曲面没有“断裂”或“洞”。从任何方向趋近某点，极限值都等于函数值。
    *   **连续 $\not\implies$ 偏导数存在**:
        *   **经典反例**：$f(x,y) = \sqrt{x^2+y^2}$
        *   这个函数是一个圆锥面，在原点是**连续**的。
        *   但在原点，它是一个尖点，无法定义偏导数。
...
| 关系                      | 一元函数         | 多元函数           |     |     |
| :---------------------- | :----------- | :------------- | --- | --- |
| **可导/可微 $\implies$ 连续** | **是**        | **是**          |     |     |
| **连续 $\implies$ 可导/可微** | **否**        | **否            |     |     |
| **连续 $\implies$ 可积**    | **是** (闭区间上) | **是** (闭区域上)   |     |     |
| **偏导数存在 $\implies$ 连续** | (无此概念)       | **否** (核心区别！)  |     |     |
| **偏导数连续 $\implies$ 可微** | (无此概念)       | **是** (重要充分条件) |     |     |

理解这些关系的关键在于认识到：
*   在一元函数中，只有一个方向，所以“可导”这个强条件足以蕴含一切。
*   在多元函数中，有无限多个方向，“偏导数存在”这个条件**太弱了**，因为它只管坐标轴方向。我们需要“可微”或“偏导数连续”这样更强的条件来保证函数在所有方向上都表现良好。
...
**3. 设 $b > a > 0$，则曲线 $y=\ln x, y=\ln a, y=\ln b$ 以及 $y$ 轴所围图形面积 $S=$ [ C ]**

*   **思路:** 这是一个典型的 Y-型区域，对 $x$ 积分比对 $y$ 积分更方便。
*   **步骤:**
    1.  **画图:** 区域是由 $y$ 轴 ($x=0$)，水平线 $y=\ln a$, $y=\ln b$ 和曲线 $y=\ln x$ 围成。
    2.  **积分方向:** 选择对 $y$ 积分（从下到上），被积函数是 $x$ 的长度。
    3.  $y$ 的范围是 $[\ln a, \ln b]$。
    4.  对于任意 $y$， $x$ 的范围是从左边界 $x=0$ 到右边界 $x=e^y$ (由 $y=\ln x$ 反解)。
    5.  面积 $S = \int_{\ln a}^{\ln b} (e^y - 0) dy = \int_{\ln a}^{\ln b} e^y dy$。
*   **选项分析:**
    (A) $\int_a^b e^x dx$ (积分变量和上下限都错)
    (B) $\int_a^b \ln x dx$ (这是对x积分，区域不对)
    (C) $\int_{\ln a}^{\ln b} e^y dy$ (**正确**)
    (D) $\int_{\ln a}^{\ln b} \ln x dx$ (积分变量混用)
...
**4. 若方程 $y''+py'+qy=0$ 的系数满足 $1+p+q=0$，则该方程有特解 [ B ]**

*   **思路:** 这是二阶常系数线性微分方程的一个结论，可以通过代入验证。
*   **验证:**
    *   (A) $y=x \implies y'=1, y''=0$. 代入得 $p+qx=0$，不恒成立。
    *   (B) $y=e^x \implies y'=e^x, y''=e^x$. 代入得 $e^x + pe^x + qe^x = (1+p+q)e^x$。因为 $1+p+q=0$，所以 $(1+p+q)e^x = 0$ 恒成立。
    *   (C) $y=e^{-x} \implies y'=-e^{-x}, y''=e^{-x}$. 代入得 $(1-p+q)e^{-x}=0$，需要 $1-p+q=0$。
    *   (D) $y=\sin x \implies y'=\cos x, y''=-\sin x$. 代入得 $(q-1)\sin x + p\cos x = 0$，不恒成立。
*   **结论:** $y=e^x$ 是一个特解。
...
**1. 设 $a>0, a\neq1$，则 $\lim_{x \to 0} \frac{a^{2x}-1}{4x} = \underline{\frac{\ln a}{2}}$**

*   **思路:** 利用重要极限 $\lim_{u \to 0} \frac{a^u-1}{u} = \ln a$ 或洛必达法则。
*   **解法1 (凑重要极限):**
    $$
    \lim_{x \to 0} \frac{a^{2x}-1}{4x} = \lim_{x \to 0} \frac{a^{2x}-1}{2x \cdot 2} = \frac{1}{2} \lim_{x \to 0} \frac{a^{2x}-1}{2x}
    $$
    令 $u=2x$，当 $x \to 0$ 时 $u \to 0$。
    $$
    = \frac{1}{2} \lim_{u \to 0} \frac{a^u-1}{u} = \frac{1}{2} \ln a
    $$
*   **解法2 (洛必达法则):**
    $$
    \lim_{x \to 0} \frac{a^{2x}-1}{4x} \overset{L'H}{=} \lim_{x \to 0} \frac{a^{2x} \ln a \cdot 2}{4} = \frac{a^0 \cdot 2\ln a}{4} = \frac{2\ln a}{4} = \frac{\ln a}{2}
    $$
...
**2. 设周期函数 $f(x)$ 在 $(-\infty, +\infty)$ 内可导，周期为 5，又 $\lim_{x \to 0} \frac{f(1)-f(1+x)}{2x} = -1$，则曲线 $y=f(x)$ 在点 $(6, f(6))$ 处的切线的斜率为 $\underline{2}$**
...
*   **步骤:**
    1.  处理给定的极限：
        $$
        \lim_{x \to 0} \frac{f(1)-f(1+x)}{2x} = -\frac{1}{2} \lim_{x \to 0} \frac{f(1+x)-f(1)}{x}
        $$
        根据导数定义，$\lim_{x \to 0} \frac{f(1+x)-f(1)}{x} = f'(1)$。
        所以，$-\frac{1}{2} f'(1) = -1 \implies f'(1) = 2$。
    2.  利用周期性：
        因为 $f(x)$ 的周期为 5，所以其导函数 $f'(x)$ 的周期也为 5。
        $f'(6) = f'(1+5) = f'(1) = 2$。
    3.  切线斜率就是该点的导数值，所以斜率为 2。
...
**3. 设 $y=y(x)$ 由方程 $y\cos x - \sin(x+y)=0$ 所确定，则 $dy = \underline{\frac{y\sin x + \cos(x+y)}{\cos x - \cos(x+y)} dx}$**

*   **思路:** 使用隐函数求导法求出 $\frac{dy}{dx}$，然后乘以 $dx$。
*   **步骤:**
    1.  方程两边对 $x$ 求导：
        $(y' \cos x + y(-\sin x)) - \cos(x+y)(1+y') = 0$
    2.  整理含有 $y'$ 的项：
        $y' \cos x - y\sin x - \cos(x+y) - y'\cos(x+y) = 0$
        $y'(\cos x - \cos(x+y)) = y\sin x + \cos(x+y)$
    3.  解出 $y'$:
        $$
        \frac{dy}{dx} = \frac{y\sin x + \cos(x+y)}{\cos x - \cos(x+y)}
        $$
    4.  $dy = \frac{dy}{dx} \cdot dx$
        $$
        dy = \frac{y\sin x + \cos(x+y)}{\cos x - \cos(x+y)} dx
        $$
...
**4. 曲线 $y = \int_0^x \tan t \, dt \quad (0 \le x \le \frac{\pi}{4})$ 的弧长 $s = \underline{\ln(\sqrt{2}+1)}$**

*   **思路:** 使用显函数弧长公式 $L = \int_a^b \sqrt{1+(y')^2} dx$。
*   **步骤:**
    1.  根据微积分基本定理，求 $y'$:
        $y' = \frac{d}{dx} \int_0^x \tan t \, dt = \tan x$
    2.  计算 $\sqrt{1+(y')^2}$:
        $\sqrt{1+(\tan x)^2} = \sqrt{1+\tan^2 x} = \sqrt{\sec^2 x} = |\sec x|$
        因为 $0 \le x \le \frac{\pi}{4}$，$\sec x > 0$，所以 $|\sec x| = \sec x$。
    3.  建立弧长积分:
        $$
    ...
    4.  计算积分:
        $$
        s = [\ln|\sec x + \tan x|]_0^{\pi/4}
        $$
        $$
        = (\ln|\sec(\pi/4) + \tan(\pi/4)|) - (\ln|\sec(0)+\tan(0)|)
        $$
        $$
        = (\ln|\sqrt{2} + 1|) - (\ln|1+0|) = \ln(\sqrt{2}+1) - \ln(1) = \ln(\sqrt{2}+1)
        $$
...
**5. 函数 $y = 1 + \frac{5x}{(x+2)^2}$ 图形的水平渐近线为 $y=\underline{1}$；铅直渐近线为 $x=\underline{-2}$**

*   **思路:**
    *   **水平渐近线:** 求 $\lim_{x \to \pm\infty} y$。
    *   **铅直渐近线:** 找使函数趋于无穷的分母为零的点。
*   **步骤:**
    1.  **水平渐近线:**
        $$
        \lim_{x \to \infty} \left(1 + \frac{5x}{(x+2)^2}\right) = 1 + \lim_{x \to \infty} \frac{5x}{x^2+4x+4}
        $$
        因为分母的最高次幂 (2) 大于分子的最高次幂 (1)，所以后面分式的极限为 0。
        $= 1 + 0 = 1$。
        所以水平渐近线是 $y=1$。
    2.  **铅直渐近线:**
        分母 $(x+2)^2 = 0$ 时，即 $x=-2$。
        当 $x \to -2$ 时，$y \to 1 + \frac{-10}{0} \to \infty$。
        所以铅直渐近线是 $x=-2$。
...
**6. 已知 $F(x)$ 是 $y=\frac{\ln x}{x}$ 的一个原函数，则 $dF(\sin x) = \underline{\frac{\ln(\sin x)}{\sin x} \cos x \, dx}$**

*   **思路:** 利用微分的定义 $df(u) = f'(u)du$。
*   **步骤:**
    1.  这里 $f(u) = F(u)$，$u = \sin x$。
    2.  我们需要 $F'(u)$。因为 $F(x)$ 是 $\frac{\ln x}{x}$ 的一个原函数，所以 $F'(x) = \frac{\ln x}{x}$。
        因此，$F'(u) = F'(\sin x) = \frac{\ln(\sin x)}{\sin x}$。
    3.  我们需要 $du$。$u = \sin x \implies du = \cos x \, dx$。
    4.  代入公式：
        $dF(\sin x) = F'(\sin x) \cdot d(\sin x) = \frac{\ln(\sin x)}{\sin x} \cdot \cos x \, dx$