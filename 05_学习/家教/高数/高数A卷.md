...
当积分区域或被积函数具有**圆形**或**径向对称性**时，使用极坐标可以极大地简化计算。
...
*   **核心转换公式**：
    1.  **坐标代换**:
        ...
        $y = r\sin\theta$
    2.  **面积微元代换 (最重要的一步)**:
        $dxdy \rightarrow r \,dr d\theta$  (注意不要漏掉这个 $r$!)
    3.  **被积函数代换**:
...
**示例：** 计算 $\iint_D (x^2+y^2) \,dxdy$，其中 $D$ 是单位圆盘 $x^2+y^2 \le 1$。
1.  **转换**：
    *   $x^2+y^2 = r^2$
    *   $dxdy = r \,dr d\theta$
    *   区域 $D$ 变为 $0 \le r \le 1, 0 \le \theta \le 2\pi$。
2.  **新积分**：
    $$
...
*   **物理意义**：
    *   如果 $f(x,y)$ 是区域 $D$ 的**面密度函数**，则 $\iint_D f(x,y) \,dA$ 就是该薄片的**总质量**。
    *   可以用来计算质心、转动惯量等。
...
*   **步骤:**
    1.  **化为标准形式:**
        ...
        $$
    2.  **判断类型:**
        这是**单叶双曲面 (Hyperboloid of One Sheet)** 的标准方程 ($\frac{x^2}{a^2} + \frac{y^2}{b^2} - \frac{z^2}{c^2} = 1$)。它的对称轴是与负号项对应的 z 轴。
    3.  **对比图像:**
        (A) 双曲抛物面（马鞍面）。
        (B) 抛物柱面。
        (C) 椭圆抛物面。
...
*(题目中为 $\triangle ABC$，应为 $\triangle OAB$)*
...
**6. 设积分区域 $\Omega$ 由不等式 $2 \le \sqrt{x^2+y^2+z^2} \le 3$ 确定，其中 $z \ge 0$，则三重积分 $\iiint_{\Omega} (x^2+y^2) dV = \underline{\frac{422}{15}\pi}$**

*   **思路:**
    积分区域是一个半球壳，被积函数含有 $x^2+y^2$，非常适合使用**球坐标**。
...
*   **步骤:**
    ...
    2.  **确定积分区域:**
        *   $2 \le \sqrt{x^2+y^2+z^2} \le 3 \implies 2 \le \rho \le 3$
        *   $z \ge 0 \implies \rho\cos\phi \ge 0 \implies \cos\phi \ge 0 \implies 0 \le \phi \le \frac{\pi}{2}$ (上半球)
        *   $\theta$ 没有限制，所以 $0 \le \theta \le 2\pi$

    3.  **建立积分:**
        $$
        I = \int_{0}^{2\pi} d\theta \int_{0}^{\pi/2} d\phi \int_{2}^{3} (\rho^2\sin^2\phi) \cdot (\rho^2\sin\phi) \,d\rho
        $$
        $$
        I = \int_{0}^{2\pi} d\theta \int_{0}^{\pi/2} \sin^3\phi \,d\phi \int_{2}^{3} \rho^4 \,d\rho
        $$

    4.  **分别计算三个定积分:**
        *   $\int_{0}^{2\pi} d\theta = 2\pi$
        *   $\int_{2}^{3} \rho^4 \,d\rho = \left[\frac{\rho^5}{5}\right]_2^3 = \frac{3^5 - 2^5}{5} = \frac{243 - 32}{5} = \frac{211}{5}$
        *   $\int_{0}^{\pi/2} \sin^3\phi \,d\phi = \int_{0}^{\pi/2} \sin^2\phi \sin\phi \,d\phi = \int_{0}^{\pi/2} (1-\cos^2\phi) \sin\phi \,d\phi$
            ...
            $= \int_{1}^{0} (1-u^2)(-du) = \int_{0}^{1} (1-u^2)du = \left[u - \frac{u^3}{3}\right]_0^1 = 1 - \frac{1}{3} = \frac{2}{3}$

    5.  **相乘得到最终结果:**
        $$
        I = (2\pi) \cdot \left(\frac{2}{3}\right) \cdot \left(\frac{211}{5}\right) = \frac{4\pi \cdot 211}{15} = \frac{844}{15}\pi
        $$
