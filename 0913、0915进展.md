...
4. 问题解决：Firth惩罚序数回归，好像没用？数据本身的“强预测性”导致完全分离？
![[Pasted image 20250915100834.png]]




5. 尝试另一种表型cognitive_symptoms_severity，仍然有10万样本的方差趋于0，导致权重趋于无穷，且该表型总样本量16.7万。这个表型甚至还没有用REGENIE处理。
![[Pasted image 20250913102601.png]]


6. 现在的问题是能不能直接嵌套STAARpipeline的框架进行关联分析，这样直接可以得到显著性的结果。（不可以，得分检验的时候不一样，而且SPA不行）
7. 但是即使是NullModel+STAARpipeline，前面的null model部分仍然会出现13万样本的权重是无限值的情况。（稳健性设置成1）
...
9. **饮酒频率：** 确认了已知基因`ADH1C`的关联，并发现了一个新的潜在关联基因`GIGYF1`。
![[Pasted image 20250915103644.png]]
GIGYF1没有找到variant？


10. OrdinalSTAAR_summary_results_no_annotations_v2里去掉的变异结果如下

![[OrdinalSTAAR_summary_results_no_annotations_v2.xlsx]]

![[Pasted image 20250915130645.png]]
![[Pasted image 20250915130726.png]]

11. OrdinalSTAAR_summary_results_no_annotations里去掉的变异结果如下




12. ordinal null model+STAARpipeline：不能处理SPA=TRUE的情况
13. ordinal null model+OrdinalSTAAR：可以处理SPA=TRUE的情况，但是存在准完全分离的问题以及p值与STAARpipeline得到的对应不上
